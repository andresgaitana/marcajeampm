...
/**
 * Búsqueda del colaborador por código, tolerante a cómo lo teclee en el terminal.
 *
 * Antes se armaba una lista de variantes ("GTA91", "GT-A91", …) y se comparaba
 * exacto contra lo GUARDADO, así que un código creado como "Pr01" nunca se
 * encontraba. Ahora se compara contra la forma canónica que calcula la propia
 * base (employee_code_canon, con índice único): mayúsculas, minúsculas, guiones
 * o espacios dan igual, y no puede haber dos colaboradores que coincidan.
 */
const codeFilter = (raw: string) => normalizeEmployeeCode(raw) || "SIN-CODIGO";
...
async function validateSupervisorOverride(
  ...
  const { data: sup } = await supabaseAdmin
    ...
    .eq("employee_code_canon", codeFilter(code))
    .maybeSingle();
...
export const markAttendance = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: employee, error: empErr } = await supabaseAdmin
      ...
      .eq("employee_code_canon", codeFilter(data.employeeCode))
      .maybeSingle();
