...
//
// Se dispara 2 veces/día por cron. El corte se pasa en el body {"corte":"AM"|"PM"}
// o se infiere por la hora de Nicaragua.
//
// Secretos (Supabase → Edge Functions → Secrets):
//   SMTP_HOST, SMTP_PORT (587/465), SMTP_SECURE ("true" p/465), SMTP_USER,
//   SMTP_PASS, SMTP_FROM ("Marcaje AMPM <marcaje@ampm.com.ni>")
//   CRON_SECRET (opcional): si está, el body debe traer {"secret": "<valor>"}.
...
type Row = { code: string; name: string; prodReal: number; prodPlan: number; mbkReal: number; mbkPlan: number };
...
Deno.serve(async (req) => {
  try {
    ...
    const corte: "AM" | "PM" = body?.corte === "AM" || body?.corte === "PM"
      ? body.corte
      : hourNow >= 5 && hourNow < 14 ? "AM" : "PM";
    ...
    if (!host || !user || !pass) {
      return Response.json({ ok: false, error: "SMTP no configurado (faltan SMTP_HOST/USER/PASS)" });
    }
    ...
    const [stores, staffing, emps, recs, uroles, uzones, smgrs] = await Promise.all([
    ...
    const staffMap = new Map((staffing as { store_id: string; prod_agents: number; mbk_agents: number }[]).map((x) => [x.store_id, x]));
    const roleById = new Map((emps as { id: string; role: string }[]).map((e) => [e.id, e.role]));
    ...
    for (const r of recs as { created_at: string; employee_id: string; store_id: string }[]) {
    ...
    const storeList = stores as { id: string; code: string; name: string; zone_id: string | null }[];
    const rowByStore = new Map<string, Row>();
    ...
    for (const s of storeList) {
      ...
      rowByStore.set(s.id, {
        ...
        prodReal: prodSet.get(s.id)?.size ?? 0,
        prodPlan: corte === "AM" ? pl.prodAm : pl.prodPm,
        mbkReal: mbkSet.get(s.id)?.size ?? 0,
        mbkPlan: corte === "AM" ? pl.mbkAm : pl.mbkPm,
      });
      if (s.zone_id) {
        if (!zoneStores.has(s.zone_id)) zoneStores.set(s.zone_id, []);
        zoneStores.get(s.zone_id)!.push(s.id);
      }
    }
    const allStoreIds = storeList.map((s) => s.id);
    ...
    for (const r of uroles as { user_id: string; role: string }[]) {
      if (r.role === "admin" || r.role === "gerente_operaciones") add(r.user_id, allStoreIds);
    }
    for (const z of uzones as { user_id: string; zone_id: string }[]) add(z.user_id, zoneStores.get(z.zone_id) ?? []);
    for (const m of smgrs as { user_id: string; store_id: string }[]) add(m.user_id, [m.store_id]);
    ...
    for (const [uid, storeIds] of recipients) {
      ...
      const rows = [...storeIds].map((id) => rowByStore.get(id)).filter(Boolean) as Row[];
      if (rows.length === 0) continue;
      rows.sort((a, b) => (a.code < b.code ? -1 : 1));
      const trs = rows.map((r) => {
        const ready = r.prodReal >= r.prodPlan && r.mbkReal >= r.mbkPlan;
        return `<tr>
          <td style="padding:6px 10px;border-bottom:1px solid #eee"><b>${r.code}</b> ${r.name}</td>
          ...
          <td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:center;color:${ready ? "#137A4B" : "#B45309"};font-weight:600">${ready ? "Listo" : "Falta"}</td>
        </tr>`;
      }).join("");
      const html = `<div style="font-family:Segoe UI,Arial,sans-serif;color:#20303B">
        ...
        <table style="border-collapse:collapse;width:100%;max-width:640px;font-size:14px">
          <thead><tr style="background:#F4F6F8">
            <th style="padding:6px 10px;text-align:left">Tienda</th>
            <th style="padding:6px 10px;text-align:right">Productos</th>
            <th style="padding:6px 10px;text-align:right">MBK</th>
            <th style="padding:6px 10px;text-align:center">Estado</th>
          </tr></thead>
          <tbody>${trs}</tbody>
        </table>
        <p style="color:#888;font-size:12px;margin-top:12px">Verde = cubierto · Ámbar = falta. "Listo" solo si Productos y MBK están cubiertos. · Marcaje AM/PM Centroamérica.</p>
      </div>`;
      try {
    ...
    return Response.json({ ok: true, corte, date: dateLabel, recipients: recipients.size, sent, failed });
  } catch (e) {
    console.error(e);
